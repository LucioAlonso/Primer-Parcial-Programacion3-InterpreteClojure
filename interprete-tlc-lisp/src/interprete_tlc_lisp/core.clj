...
(defn repl
  ...
  ([]
   ...
   (repl '(add add append append nth nth cond cond cons cons de de env env equal equal
...
(defn igual? [a b] 
		(cond
				...
				(and (seq? a) (seq? b)) (= (.toUpperCase (str a)) (.toUpperCase (str b)) )
				(= a b) true
				(not (= a b)) false
		)
)
...
(defn error? [a] 
		(cond
				(and (seq? a) (= (.toUpperCase (str (first a))) "*ERROR*")) true
				true false
		)
)
...
(defn actualizar-amb [a b c] 
		(cond
				(error? c) a
				(igual? nil a) (list b c)
				(every? false? (map (partial igual? b) (map first (partition 2 a)))) (concat a [b c])
				:else (apply concat (replace {(concat (list (nth a (* 2(count (take-while false? (map (partial igual? b) (map first (partition 2 a)))))))) (list (buscar b a))) (concat (list b) (list c))} (partition 2 a)))
		)
)
...
(defn buscar [a b]
		(if (every? false? (map (partial igual? a) (map first (partition 2 b)))) (concat '(*error* unbound-symbol) (list a)) (nth b (inc (* 2 (.indexOf (map (partial igual? a) (map first (partition 2 b))) true)))))
)
...
(defn fnc-append [a] 
		(if (= 2 (count a)) 
				(if (and (or (seq? (first a)) (igual? nil (first a))) (or (seq? (second a)) (igual? nil (second a)) )) 
					 ( if (and (igual? nil (first a)) (igual? nil (second a)) )
					 		nil
...
(defn fnc-reverse [a]
		(cond
				...
				(igual? nil a) (controlar-aridad a 1)
				(not (seq? (first a))) (concat '(*error* list expected) a)
				(seq? (first a)) (reverse  (first a))
		)
)						
...
(defn evaluar-de [a b]
  (cond
  		(and (= (first '(de)) (first a)) (symbol? (second a)) (seq? (first (nnext  a)))) (concat  (list (second a)) (list (concat  (concat b (list (second a))) (list (concat (list 'lambda) (nthrest a 2))))))
  		(and (= (first '(de)) (first a)) (not (seq? (first (nnext  a))))) (concat (list (concat '(*error* list expected) (list (first (nnext a))))) (list b))
  		(and (= (first '(de)) (first a)) (igual? nil (nnext a))) '(*error* cannot-set nil)
  		(and (= (first '(de)) (first a)) (igual? nil (second a))) (concat (list (concat '(*error* cannot-set) (list (second a)))) (list b))
  		(and (= (first '(de)) (first a)) (not (symbol? (second a)))) (concat (list (concat '(*error* symbol expected) (list (second a)))) (list b))
  )
)
...
(defn evaluar-if [a b c]
		(cond
				(seq? (second a)) (if (igual? 't (first (evaluar (second a) b c))) (evaluar (first (nnext a)) b c) (evaluar (last a) b c) )
				(or (= 2 (count a)) (and (= 3 (count a)) (igual? nil (second a)))) (evaluar nil b c) 
				(and (<= 4 (count a)) (igual? nil (second a))) (evaluar (last a) b c)
...
;evalua mientras obtenga nil

(defn evaluar-or [a b c]
		(let [aux (evaluar (second a) b c)]
				(cond
						(and (not (igual? (nnext a) nil)) (or (igual? (first aux) nil) (error? (first aux)))) (evaluar-or (concat '(or) (nnext a)) (second aux) c)
						:else aux
				)
		)
)
...
(defn evaluar-setq [a b c]
		 (cond
		 		...
		 		(not-every? false? (map (partial igual? nil) (map first (partition 2 (next a))))) (concat (list '(*error* cannot-set nil)) (list b))
		 		(not-every? true? (map symbol? (map first (partition 2 (next a))))) (concat (list (concat '(*error* symbol expected) (list (nth (map first (partition 2 (next a))) (.indexOf (map symbol? (map first (partition 2 (next a)))) false))))) (list b))
					(igual? nil (second a)) (concat (list (concat '(*error* list expected) (list (second a)))) (list b))
		 		;Casos de NO error

		 		(= 3 (count a)) (let [aux (evaluar (last a) b c)] (conj (list (first aux) (actualizar-amb (second aux) (second a) (first aux)))))
				 (= 2 (count a))		(concat (list '(*error* list expected nil)) (list b))			
				 (= 1 (count a))		(concat (list '(*error* list expected nil)) (list b))			
				 (<= 4 (count a)) (evaluar (concat '(setq) (drop 3 a)) (actualizar-amb b (second a) (first (evaluar (second (next a)) b c))) c) 																																																							
		 )
)
...
;(defn fnc-nth 
;		([] '(*error* too-few-args))
;		([a] '(*error* too-few-args))
;		([a b] 
;				(cond
;						(not (number? b)) '(*error* number-expected)
;						(or (>= b (count a)) (neg? b)) '(*error* out-of-bounds)
;					 :else (nth a b)
;				)
;		)
;		([a b & more] '(*error* too-many-args)) 
;)

(defn fnc-nth [a]
	(cond
		(empty? (next a)) '(*error* too-few-args)
		(not (number? (second a))) '(*error* number-expected)
		(not (list? (first a))) '(*error* list-expected)
		(or (> (second a) (count a)) (neg? (second a))) '(*error* out-of-bounds)
	 :else (nth (first a) (second a))
	)
)
